...
function detectPetraWallet() {
  if (typeof window !== 'undefined' && (window as any).aptos) {
    console.log('[Rugsense/content] Petra Wallet detected in content script');
    return true;
  }
  return false;
}
...
function injectInpageScript() {
  try {
    ...
    script.onload = () => {
      ...
      // Petra wallet varsa bildir
      if (detectPetraWallet()) {
        console.log('[Rugsense/content] Petra Wallet integration active');
      }
    };
    script.onerror = () => {
      console.warn(
